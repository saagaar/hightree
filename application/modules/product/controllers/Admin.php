...

		{
			...
			// {
				...
				if($this->input->post('category',TRUE) && $this->input->post('category',TRUE)!=''){
					...
					$cf_old_files = $this->admin_product_model->get_custom_fields_meta_files($product_id);

					//echo "<pre>"; print_r($cf_files); echo "</pre>"; exit;

				}else{

					$this->data['cat_id'] = $this->data['product']->sub_cat_id!=0?$this->data['product']->sub_cat_id:$this->data['product']->cat_id;

					$cf_post_value = $this->admin_product_model->get_custom_fields_meta_values($product_id);
				...
				$this->data['categories']=$this->account_module->get_data('product_post_categories',array('product_id'=>$product_id));
...
public function cancelbid($product_id,$bid_id)
	{
		try{
			...
			if($data){
				...
				$this->session->set_flashdata('success_message', 'The Bid Deleted sucessfully');
				redirect('/'.ADMIN_DASHBOARD_PATH.'/product/view_bids/'.$product_id, 'refresh');
			}else{
				$this->session->set_flashdata('error_message', 'The Bid Deletion Failed');
				redirect('/'.ADMIN_DASHBOARD_PATH.'/product/view_bids/'.$product_id, 'refresh');
			}
