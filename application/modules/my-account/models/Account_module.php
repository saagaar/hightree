...

{
...

	{
		...
		if(isset($static_field['auction_start_time']) && $static_field['auction_start_time']['display']=='1'){ 
			...
			array_push($static_field_validation_arr,array('field' => 'auc_start_time', 'label' => 'Auction Start Time', 'rules' => 'required|callback_check_auc_start_date'));
		...
		if(isset($static_field['bid_decrement']) && $static_field['bid_decrement']['display']=='1'){ 
			...
			array_push($static_field_validation_arr,array('field' => 'bid_decrement', 'label' => 'Bid Decrement', 'rules' => 'required|greater_than[0]'));
	...
	public function get_pname($product_id){
...

	{
...

		{
			...
			$html = $cat_id>0?'<h4><i class="fa fa-plus" aria-hidden="true"></i>&nbspAdditional Information</h4>':'<h4><i class="fa fa-paperclip" aria-hidden="true">&nbsp;</i>Attach Supplementary Documents</h4>';
	...
	public function insert_new_product($payment_type)
	{
		...
		$time=$hour.':'.$minute;
		if($payment_type=='unlimited' || $payment_type=='one_post')
		{
			$payment='paid';	
		}
		else{
		...
		if($product_id)
		{
			//storing multiple categories for product
			foreach($categories as $categoriesval)
			{
				$bulkcat[]=array('user_id'=>$user_id,'category_id'=>$categoriesval,'product_id'=>$product_id);
			}
	...
	if($status = '2' && $product_status != '2')
		{
			...
			$product_data['auc_end_time'] = $this->general->get_end_date_with_time($this->general->get_local_time('now'), $days,$hour,$minute);
		...
		$meta_product=$this->db->dbprefix('meta_products');
		$meta_field=$this->db->dbprefix('meta_fields');
		$sql = 'DELETE MP FROM '. $meta_product.' MP LEFT JOIN '.$meta_field.' MF ON MP.meta_fields_id = MF.id WHERE MP.product_id = '.$product_id.' AND MF.type!="FILE"';
...
// to upload custom fields files
...

	{
        ...
        $this->db->order_by($sortcolumn,$orderby);
		...
		try{
			...
			if(!$table) throw new Exception("Error occured to perform operation", 1);
				...
				$id=$this->db->insert($table, $data);
					...
					if($id) return $this->db->insert_id();
...
// to retrieve membership package for both buyer and supplier
...

	{
			...
			// die($this->input->post('order_data')); 

// echo $this->input->post('order_data');
		...
		//die($order."-".$category."-".$subcategory);



		$this->db->select('P.id, product_code, name, description, auc_start_time, auc_end_time, status');
		$this->db->from('products P');
		$this->db->join('product_post_categories PC','P.id=PC.product_id','left');
		$this->db->where('auc_end_time >', $current_time); // auction end date > current date time
		...
		if($subcategory && $subcategory > 0)
			$this->db->where('PC.category_id', $subcategory);
		else{
...

	{
		...
		$this->db->select('PB.id as bid_id,P.id as prd_id, P.product_code, P.name, PB.user_bid_amt, PB.bid_date');
...

	{
		...
		$this->db->from('product_winner as w');

		$this->db->join('products as p','w.product_id=p.id');

		$this->db->join('members as m','m.id=p.seller_id');
...

    {
        ...
        // added by sagar
 	   // invite supplier
    public function invite_suppliers($product_id,$emaildata=false,$messagedata=false)
    {
    	$invitemail=$this->input->post('user_email');
    	$message=$this->input->post('message');
    	$user_email= isset($invitemail)?$invitemail:$emaildata;
    	$message_data=isset($message)?$message:$messagedata;
		...
		$productdetail=$this->db->get_where('products',array('id'=>$product_id));
		...
		if($emailsend && (!$emaildata) ){
			$id=$this->db->insert('product_invitation', $invitation_data);
			if($id) return $this->db->insert_id();
			else return false;
		}elseif($emailsend){
			return true;
...

    {
    	...
    	try{
    		...
    		$this->db->delete('member_expertise', array('user_id' => $user_id)); 
    		...
    		$this->db->insert_batch('member_expertise', $data);
...

    {
    	...
    	$query = $this->db->get_where('member_expertise', array('user_id'=>$user_id));
