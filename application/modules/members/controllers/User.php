...
class User extends CI_Controller {
    ...
    public function create_auction() {
        ...
        if ($auction_post_validity === FALSE) {
            $this->session->set_flashdata('test_message', 'Your balance is not available');

            redirect(site_url('/' . MY_ACCOUNT . 'buyer_packages'));
            exit;
        }
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (AUCTION_POST_ACTIVATION == '1') {
                ...
                redirect(site_url('/' . MY_ACCOUNT . 'create_auction'));
                exit;
            } else {

                $this->session->set_flashdata('success_message', 'Item Added Successfully.');

                redirect(site_url('/' . MY_ACCOUNT . 'invitation/' . $product_id), 'refresh');
                exit;
            }
    ...
    public function invitation($product_id = '') {
        ...
        $this->data['product_name']=$this->account_module->get_pname($product_id);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($change) {
                    ...
                    $this->session->set_flashdata('success_message', "Your password is changed. Please re login to access your account");
    ...
    public function buyer_packages() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $this->session->set_flashdata('test_message','No package is chosen.');
                redirect(site_url('/' . MY_ACCOUNT . 'buyer_packages'), 'refresh');
                exit;
            }
        } else {
    ...
    public function make_winner($product_id = false, $user_id = false) {
        ...
        try {
            ...
            if (!($product_id && $user_id))
                throw new Exception("Error in data variables", 1);
    ...
    public function send_message($product_id = false, $bidid = false) {
        ...
        try {
            ...
            if ((!$bidid) || (!$product_id))
                throw new Exception("Error in data selection", 1);
            ...
            if ($valid < 1)
                redirect(site_url('/' . MY_ACCOUNT . 'bid_details/' . $product_id));
                    ...
                    if ($_FILES['attachmentcommunication'] && $lastid) {
                        ...
                        if ($upload) {
                            ...
                            if ($id)
                                ...
                                $message = 'Attchment couldn\'t  be upload';
                        }
    ...
    public function member_profile($user_id = false) {
        ...
        try {
            ...
            if (!$user_id)
                throw new Exception("Sorry,No user selected", 1);
    ...
    public function saverating() {
        ...
        if ($id)
            echo json_encode(array('success' => 'Thank you for your time!'));
        else
            echo json_encode(array('error' => 'Sorry something went wrong please try once'));
    }
    ...
    public function getbidbyproduct($product_id = false) {
        ...
        try {
            ...
            if (!$product_id)
                throw new Exception("No product selected", 1);
    ...
    public function supplier_packages() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $this->session->set_flashdata('pay_msg','No package chosen.');
                redirect(site_url('/' . MY_ACCOUNT . 'supplier_packages'), 'refresh');
                exit;
            }
        } else {
    ...
    public function purchase_checkout($product_id) {
        ...
        if (!is_numeric($product_id)) {
            ...
            redirect(site_url('/' . MY_ACCOUNT . 'my_auctions'), 'refresh');
            exit;
        }
    ...
    public function attachment($product_id) {
        ...
        if ($product_id) {
            ...
            if ($attachment) {
                ...
                redirect(site_url('/' . MY_ACCOUNT . 'auction_detail/' . $product_id), 'refresh');
                exit;
            }
        } else {
