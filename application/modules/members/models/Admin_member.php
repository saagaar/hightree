...
class Admin_member extends CI_Model 
{
	...
	{
		$this->db->select('PB.id as bid_id,PB.id as bid_id,P.id as prd_id,P.budget,PB.user_bid_amt, P.product_code, P.name, PB.user_bid_amt, PB.bid_date');
		$this->db->from('product_bids PB');
		$this->db->join('products P', 'P.id = PB.product_id','left');
